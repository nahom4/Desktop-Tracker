...
interface HealthBarsProps {
  ...
  period: "day" | "week";
  onCategoryClick?: (category: CategoryHealth) => void;
}

export function HealthBars({ title = "Category health", health, period, onCategoryClick }: HealthBarsProps) {
  ...
  return (
    <div className="rounded-xl border border-border bg-surface/40 p-5">
      <div className="flex items-baseline justify-between mb-4">
        ...
        <span className="text-xs text-subtle">target per {period}</span>
      </div>
      <div className="space-y-3">
        {tracked.map((h) => {
          const progress = h.progress ?? 0;
          const pct = Math.round(progress * 100);
          const targetLabel = h.targetMs ? formatDuration(h.targetMs) : "no target";
          const remaining = h.targetMs ? Math.max(0, h.targetMs - h.actualMs) : 0;
          const clickable = period === "week" && Boolean(onCategoryClick);
          const content = (
            <>
                ...
                <div className="tabular-nums text-muted">
                  ...
                  {h.targetMs && (
                    <span className="ml-2 text-xs text-subtle">
                      {progress >= 1 ? `+${formatDuration(h.actualMs - h.targetMs)} over` : `${formatDuration(remaining)} to go`}
                    </span>
                  )}
                </div>
              </div>
              <div className="h-2 w-full rounded-full bg-elevate/70 overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.min(100, pct)}%`, backgroundColor: h.color, opacity: progress >= 1 ? 1 : 0.7 }}
                />
              </div>
            </>
          );
          return clickable ? (
            ...
            <div key={h.category}>{content}</div>
          );
        })}
      </div>
    </div>
