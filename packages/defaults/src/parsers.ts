...
/**
 * Bare binary name: lowercased, with the Windows `.exe` suffix dropped, so one
 * parser covers "Code.exe" on Windows and "code" on Linux.
 */
function baseExe(exe: string): string {
  return exe.toLowerCase().replace(/\.exe$/, "");
}

const VSCODE_EXES = new Set([
  "code",
  "code - insiders",
  "code-insiders",
  "cursor",
...
const vscodeParser: TitleParser = (exe, title) => {
  if (!VSCODE_EXES.has(baseExe(exe))) return null;
...
const jetbrainsParser: TitleParser = (exe, title) => {
  if (!/^(idea|pycharm|webstorm|goland|rustrover|clion|rider|androidstudio|studio)(64)?$/.test(baseExe(exe))) return null;
  const parts = title.split(" - ");
  if (parts.length < 2) return null;
  return parts[parts.length - 2]?.trim() || null;
};
...
const visualStudioParser: TitleParser = (exe, title) => {
  if (baseExe(exe) !== "devenv") return null;
...
const figmaParser: TitleParser = (exe, title) => {
  if (baseExe(exe) !== "figma") return null;
...
const slackParser: TitleParser = (exe, title) => {
  if (baseExe(exe) !== "slack") return null;
  const parts = title.split(" - ");
...
const discordParser: TitleParser = (exe, title) => {
  if (baseExe(exe) !== "discord") return null;
...
const obsidianParser: TitleParser = (exe, title) => {
  if (baseExe(exe) !== "obsidian") return null;
  const parts = title.split(" - ");
  if (parts.length < 3) return null;
  return parts[parts.length - 2]?.trim() || null;
};
